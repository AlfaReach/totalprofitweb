...
export function Header() {
  ...
  return (
    <header className="bg-background/95 backdrop-blur-md border-b border-border">
      <nav className="relative mx-auto flex max-w-7xl items-center px-6 py-4 lg:px-8" aria-label="Основна навигация">
        <Link href="/" className="flex items-center" aria-label="Total Profit — начало">
          <Image
        ...
        <div className="hidden md:flex items-center gap-2 ml-auto">
          ...
          <Button size="sm" asChild>
            <Link href="/#contact-form">Безплатен анализ</Link>
          </Button>
        </div>

        <div className="flex md:hidden ml-auto">
          <button type="button" className="inline-flex items-center justify-center p-2" onClick={() => setMobileMenuOpen(!mobileMenuOpen)} aria-expanded={mobileMenuOpen}>
          ...
          <div className="space-y-1 px-6 py-4">
            ...
            <div className="pt-4">
              <Button className="w-full" asChild>
                <Link href="/#contact-form" onClick={() => setMobileMenuOpen(false)}>Безплатен анализ</Link>
              </Button>
            </div>
          </div>
        </div>
